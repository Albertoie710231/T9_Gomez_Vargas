...
static void app_c0_task(void *pvParameters)
{
    ...
    if (xTaskCreate(receive_c0_task, "RECEIVE_C0_TASK", configMINIMAL_STACK_SIZE + 128, NULL, tskIDLE_PRIORITY + 1, NULL) !=
    	pdPASS)
	{
	...
	if (xTaskCreate(write_c0_task, "WRITE_C0_TASK", configMINIMAL_STACK_SIZE + 128, NULL, tskIDLE_PRIORITY + 1, NULL) !=
		pdPASS)
	{
		PRINTF("Task creation failed!.\r\n");
		while (1)
